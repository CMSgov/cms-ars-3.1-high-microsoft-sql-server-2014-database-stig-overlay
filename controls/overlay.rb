...
include_controls 'microsoft-sql-server-2014-database-stig-baseline' do
  ...
  control 'V-67357' do
    desc 'Authentication with a CMS-approved PKI certificate does not necessarily imply authorization to access the database and all its contents. To mitigate the risk of unauthorized access to sensitive information by entities that have been issued certificates by CMS-approved PKIs, all CMS systems, including SQL Server databases, must be properly configured to implement access control policies. 
  ...
  control 'V-67361' do
    title 'Where SQL Server Audit is in use at the database level, SQL Server must allow only the ISSO (or individuals or roles appointed by the ISSO) to select which auditable events are to be audited at the database level.'
  end
  ...
  control 'V-67385' do
    title 'Symmetric keys (other than the database master key) must use a CMS certificate to encrypt the key.'
    ...
    If the certificate specified is not a CMS PKI certificate, this is a finding.'
  end
  ...
  control 'V-67407' do
    title 'Time stamps in database tables, intended for auditing or activity-tracking purposes, must include both date and time of day, with a minimum granularity of one hundred milliseconds (1/10th of a second).'
    ...
    DATETIMEOFFSET - as datetime2, together with local offset from UTC DATE, together with TIME (same precision considerations as for datetime2)
    DATETIME - precision 1/10th of a second Character-string data types allowing for at least 20 characters are also permissible, but not recommended.
    ...
    If these are not defined and maintained to include date and time of day, accurate to a granularity of one hundred milliseconds (1/10th of a second) or finer, this is a finding.'
    desc 'fix', 'Modify applications and/or column/field definitions so that the time stamps in audit-trail and activity-tracking columns/fields in application data include date and time of day, to a granularity of one hundred milliseconds (1/10th of a second) or finer, and are recorded accurately.'
  end
